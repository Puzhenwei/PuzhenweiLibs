...
final class BitmapUtil {
     ...
     * <p/>
     * <br>
     * <b>说明</b> 使用方法： 首先你要将Options的inJustDecodeBounds属性设置为true，BitmapFactory.decode一次图片 。
     * 然后将Options连同期望的宽度和高度一起传递到到本方法中。 之后再使用本方法的返回值做参数调用BitmapFactory.decode创建图片。
     * <p/>
     * <br>
     * <b>说明</b> BitmapFactory创建bitmap会尝试为已经构建的bitmap分配内存 ，这时就会很容易导致OOM出现。为此每一种创建方法都提供了一个可选的Options参数
     * ，将这个参数的inJustDecodeBounds属性设置为true就可以让解析方法禁止为bitmap分配内存 ，返回值也不再是一个Bitmap对象，
     * 而是null。虽然Bitmap是null了，但是Options的outWidth、 outHeight和outMimeType属性都会被赋值。
     *
     ...
     * @param outPadding If not null, return the padding rect for the bitmap if it exists, otherwise set padding to
     *            [-1,-1,-1,-1]. If no bitmap is returned (null) then padding is unchanged.
     * @param reqWidth 目标宽度
     * @param reqHeight 目标高度
     */
     ...
     * dimension android.R.dimen.launcher_application_icon_size.
     * <p/>
    ...
    /**
     * A safer decodeStream method rather than the one of {@link BitmapFactory} which will be easy to get OutOfMemory
     * Exception while loading a big image file.
     *
     * @param uri 压缩相册返回的照片
     * @return
