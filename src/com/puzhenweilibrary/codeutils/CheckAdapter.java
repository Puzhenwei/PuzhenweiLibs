 ...
 * <h2>选择适配器</h2> <br>
 * 准备工作： <br>
 * 第一步：创建你的Item并实现CheckAdapter.CheckItem接口 <br>
 * 第二步：创建你的适配器并继承于CheckAdapter并同时通过泛型指定Item类型 <br>
 * 第三步：在构造函数中传给父类Item集合 <br>
 * 第四步：在getView()方法中调用handleCheckBox()方法处理CheckBox <br>
 * 第五步：在ListView的OnItemClickListener中调用clickIitem()方法处理点击事件 <br>
 * 至此，全部准备工作已经完结 <br>
 * <br>
 * 接下来你可以： <br>
 * 调用setSingleMode()方法修改选择模式为单选模式（默认是多选模式） <br>
 * 调用enableCheckMode()方法开启选择模式 <br>
 * 调用cancelCheckMode()方法取消选择模式 <br>
 * 调用isEnableCheckMode()方法判断是否正处于选择模式 <br>
 * 调用getCheckedItems()方法获取全部选中的项 <br>
 * 调用deleteCheckedItems()方法删除全部选中的项
 */
