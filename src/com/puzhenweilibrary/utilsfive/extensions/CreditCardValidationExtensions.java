...
import com.puzhenweilibrary.utilsfive.creditcard.CardType;
...
final public class CreditCardValidationExtensions {
    
    /**
     * number of characters to determine length
     */
    public static final int CC_LEN_FOR_TYPE = 4;
    
    private CreditCardValidationExtensions()
    ...
    public static boolean isValidCvv(@Nullable
    final String cvv) {
        return !isEmpty(cvv) && cvv.matches("^[0-9]{3,4}$");
    }
    
    public static boolean isValidDateExpirationDate(@Nullable
    final Date date, @NonNull
    final Locale locale) {
        if (date == null)
            return false;
        ...
        return creditCardYear > currentYear || (creditCardYear >= currentYear && creditCardMonth >= currentMonth);
    }
    ...
    public static CardType findCardType(String number) {
        ...
        for (CardType type : CardType.values()) {
            if (type.typeRegex != null) {
                Pattern pattern = type.typeRegex;
    ...
    public static boolean isValidNumber(String number) {
        ...
        if (cardType == null || cardType.fullRegex == null)
            return false;
        
        Pattern pattern = cardType.fullRegex;
    ...
    public static String formatForViewing(String enteredNumber, CardType type) {
        ...
        String ret = String.format("%s%s%s%s%s%s%s", segment1, gaps.get(0), segment2, gaps.get(1), segment3, gaps.get(2), segment4);
