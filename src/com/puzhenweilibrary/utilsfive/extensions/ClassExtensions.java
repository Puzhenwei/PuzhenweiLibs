...
import static com.puzhenweilibrary.utilsfive.MathExtensions.randInt;
...
final public class ClassExtensions {
    ...
    final Class first, @NonNull
    final Class second) {
        return first.getCanonicalName().equals(second.getCanonicalName());
    }
    
    public static <T extends Enum<T>> T getRandom(@NonNull
    final Class<T> enumType) {
        final T[] values = values(enumType);
        return values[randInt(0, values.length - 1)];
    }
    
    public static <T extends Enum<T>> T[] values(@NonNull
    final Class<T> enumType) {
        return enumType.getEnumConstants();
    }
    
    public static <T extends Enum<T>> T[] valuesFromRawType(@NonNull
    ...
    public static <E extends Enum<E>> E[] getSortedEnum(@NonNull
    final Class<E> type) {
    ...
    @NonNull
    public static <Instance> Instance newInstance(@NonNull
    final Class clazz) {
    ...
    @Nullable
    @SuppressWarnings("unchecked")
    public static <Instance, ConstructorParam> Instance newInstanceWith(@NonNull
    final Class clazz, @NonNull
    final ConstructorParam constructorParam) {
        Instance instance = null;
        try {
    ...
    static void setFinalStatic(@NonNull
    final Field field, final Object newValue)
        throws Exception {
