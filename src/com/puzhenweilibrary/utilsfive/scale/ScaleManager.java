package com.puzhenweilibrary.utilsfive.scale;
...
public class ScaleManager {
    ...
    @Nullable
    public Matrix getScaleMatrix(@NonNull
    final ScalableType scalableType) {
        switch (scalableType) {
            case NONE:
                return getNoScale();
    ...
    @NonNull
    private Matrix getMatrix(final float sx, final float sy, @NonNull
    final PivotPoint pivotPoint) {
        switch (pivotPoint) {
            case LEFT_TOP:
    ...
    @NonNull
    private Matrix getFitScale(@NonNull
    final PivotPoint pivotPoint) {
    ...
    @NonNull
    private Matrix getOriginalScale(@NonNull
    final PivotPoint pivotPoint) {
        final float sx = mVideoSize.getWidth() / (float)mViewSize.getWidth();
        final float sy = mVideoSize.getHeight() / (float)mViewSize.getHeight();
        return getMatrix(sx, sy, pivotPoint);
    ...
    @NonNull
    private Matrix getCropScale(@NonNull
    final PivotPoint pivotPoint) {
        float sx = (float)mViewSize.getWidth() / mVideoSize.getWidth();
        float sy = (float)mViewSize.getHeight() / mVideoSize.getHeight();
