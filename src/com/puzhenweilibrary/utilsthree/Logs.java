    ...
    public static void chunkLog(int priority, String tag, String text) {
        ...
        if (text.length() > 4000) {
            ...
            for (int i = 0; i <= chunkCount; i++) {
                ...
                if (max >= text.length()) {
                    if (hasCrashlytics) {
                        logWithCrashlytics(getInstance().getCrashlyticsClass(), Log.DEBUG, tag, "CL: " + log.substring(4000 * i));
                    } else {
                        Log.println(priority, tag, log.substring(4000 * i));
                    }
                } else {
                    if (hasCrashlytics) {
                        logWithCrashlytics(getInstance().getCrashlyticsClass(), Log.DEBUG, tag, "CL: " + log.substring(4000 * i, max));
                    } else {
    ...
    public static void logWithCrashlytics(Class crashlyticsClass, int priority, String tag, String text) {
        try {
            ...
            for (Method m : methods) {
                if (m.getName().equals("log") && m.getParameterTypes()[0].toString().equals("int")) {
