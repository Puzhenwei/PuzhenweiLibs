...
public class ThreadPoolUtils {
     ...
     * 执行给定的任务
     * <p>
     * 当所有任务完成时，返回保持任务状态和结果的Future列表。 返回列表的所有元素的{@link Future#isDone}为{@code true}。 注意，可以正常地或通过抛出异常来终止已完成任务。
     ...
     * 执行给定的任务
     * <p>
     * 当所有任务完成或超时期满时(无论哪个首先发生)，返回保持任务状态和结果的Future列表。 返回列表的所有元素的{@link Future#isDone}为{@code true}。 一旦返回后，即取消尚未完成的任务。
